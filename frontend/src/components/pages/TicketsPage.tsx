...
const TicketsPage = () => {
  ...
  Object.entries(ticketsByScreening).forEach(
    ([screeningId, screeningTickets]) => {
  ...
  upcomingScreenings.sort(
    (a, b) =>
      new Date(a[1][0].screening.start_time).getTime() -
      new Date(b[1][0].screening.start_time).getTime(),
  );
  pastScreenings.sort(
    (a, b) =>
      new Date(b[1][0].screening.start_time).getTime() -
  ...
  const confirmReissue = async () => {
    ...
    const reissuePromises = reissueTickets.map((t) =>
      dbApi.tickets.reissue(t.id),
    );

    toast.promise(Promise.all(reissuePromises), {
      loading:
        reissueTickets.length > 1
          ? `Reissuing ${reissueTickets.length} tickets...`
          : "Reissuing ticket...",
      success:
        reissueTickets.length > 1
          ? "Tickets reissued! Check your email."
          : "Ticket reissued! Check your email.",
      error:
        reissueTickets.length > 1
          ? "Failed to reissue tickets."
          : "Failed to reissue ticket.",
    });
  ...
  const renderScreening = ([screeningId, screeningTickets]: [
    string,
    RichTicket[],
  ]) => {
    const screening = screeningTickets[0].screening;
    ...
    return (
      <div
        key={screeningId}
        className="bg-base-100 p-6 rounded-2xl shadow-lg flex flex-col sm:flex-row gap-6 hover:shadow-xl transition-shadow"
      >
        {/* Movie Poster */}
        <div className="shrink-0 w-full sm:w-32 md:w-48 aspect-2/3 rounded-xl overflow-hidden bg-base-300">
          {movie.poster_url ? (
            ...
            <div className="w-full h-full flex items-center justify-center opacity-50 text-sm">
              No Poster
            </div>
          )}
        </div>
              ...
              {screeningTickets.map((ticket) => {
                ...
                return (
                  ...
                  >
                    ...
                    <ul
                      tabIndex={0}
                      className="dropdown-content z-1 menu p-2 shadow bg-base-200 rounded-box w-52 mt-1"
                    >
                      <li>
                        <button
      ...
      <div className="w-full flex flex-col gap-4 mx-auto py-4">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-base-content/70">
            View your tickets for upcoming and past screenings
          </p>
        </div>
        {tickets.length === 0 ? (
            ...
            >
              ...
              <div className="collapse-content flex flex-col gap-6 pt-4">
                {upcomingScreenings.length > 0 ? (
                  ...
                  <p className="opacity-70 italic py-4">
            ...
            >
              ...
              <div className="collapse-content flex flex-col gap-6 pt-4">
                {pastScreenings.length > 0 ? (
                  ...
                  <p className="opacity-70 italic py-4">No past screenings.</p>
                )}
              </div>
            </details>
          </div>
        ...
        <div className="animate-subtle-zoom-fade">
          ...
          <Card>
            <div className="w-full p-2 text-center font-bold">
              {reissueTickets.length > 1
                ? `Reissue ${reissueTickets.length} Tickets?`
                : "Reissue Ticket?"}
            </div>
