...
class CinemaAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.audience_user = mgr.create_user(username='audience', email='audience@test.com', password='password')
        self.staff_user = mgr.create_user(username='staff', email='staff@test.com', password='password')
        self.staff_user.groups.add(self.staff_group)
        self.admin_user = mgr.create_user(username='admin', email='admin@test.com', password='password')
    ...
    def test_audience_cannot_create_movie(self):
        ...
        data = {'title': 'Interstellar', 'duration': '02:49:00', 'release_date': '2014-11-07'}
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_staff_can_view_all_tickets(self):
        # Create a ticket for audience
        Ticket.objects.create(client=self.audience_user, screening=self.screening, seat=self.seat)
    ...
    def test_staff_can_validate_ticket(self):
        ticket: Any = Ticket.objects.create(client=self.audience_user, screening=self.screening, seat=self.seat)
        cl: Any = self.client
        cl.force_authenticate(user=self.staff_user)
    ...
    def test_audience_sees_only_own_tickets(self):
        ...
        mgr: Any = User.objects
        other_user = mgr.create_user(username='other', email='other@test.com', password='password')
        other_seat = Seat.objects.create(hall=self.hall, row_label="A", seat_number=2, grid_x=1, grid_y=2)
        Ticket.objects.create(client=other_user, screening=self.screening, seat=other_seat)
